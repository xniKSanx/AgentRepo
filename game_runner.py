...
from logging_contract import (
    ...
    write_jsonl_sidecar, LOG_VERSION,
)
...
class GameLogger:
    ...
    def _log_header(self):
        self.entries.extend(format_gui_header(self.config))
    ...
    def log_move(self, round_num, agent_index, agent_name, operator, env):
        self.entries.append(format_move_line_gui(round_num, agent_index, agent_name, operator))
        for i, robot in enumerate(env.robots):
            pkg_info = ""
            if robot.package is not None:
    ...
    def save(self, directory="game_logs"):
        ...
        with open(filepath, "w") as f:
            f.write("\n".join(self.entries) + "\n")
        # Write JSONL sidecar alongside the text log
        jsonl_fp = jsonl_path_for(filepath)
        header = {
            "seed": self.config["seed"],
            "count_steps": self.config["count_steps"],
            "agent_names": [agent0, agent1],
            "time_limit": self.config["time_limit"],
        }
        write_jsonl_sidecar(jsonl_fp, header, self._jsonl_moves, self._jsonl_result)
        return filepath
